...
function I18n(opts){
  ...
  if(opts){
    ...
    self.localedir     = opts.localedir

    // Language of the original text
    self.defaultLocale = opts.defaultLocale
    // Valid languages
    self.locales       = opts.locales
    // Current locale, defaults to defaultLocale
    self.locale        = opts.locale || opts.defaultLocale

    // Clean not used
    self.cleanUnused   = opts.clean  || false
...
I18n.prototype.updateCatalogs = function(){
  /* TODO CONTROL THAT EXTENSION .yml? .yaml?*/
  var self = this
  _.each(this.locales, (l)=>{
    fs.writeFile(
      path.join(self.localedir, l+'.yaml'),
